...
export default function LiquidGlass() {
    ...
    useFrame((state) => {
        ...
        // Very slow, subtle organic movement
        const slowWarp = Math.sin(time * 0.2) * 0.05;

        // Apply rotation to bend light passing through differently based on position
        meshRef.current.rotation.x = Math.PI / 16 + slowWarp;
        meshRef.current.rotation.y = -Math.PI / 32 - slowWarp * 1.5;

        // Gentle z movement to slightly change refraction intensity
        meshRef.current.position.z = Math.sin(time * 0.4) * 0.1;
